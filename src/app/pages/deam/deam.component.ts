...
@Component({
  selector: 'app-deam',
  templateUrl: './deam.component.html',
  styleUrls: ['./deam.component.css']
})
export class DeamComponent implements OnInit {
  a:any;
   options:any;
   res:any;
   display:any;
  ...
  uploadata(books:any){
     ...
     console.warn(books);
     this.add.deam(books).subscribe((res)=>{
  ...
  ngOnInit(): void {
    this.display=this.add.getdeam().subscribe(res=>{
      this.product = res;
