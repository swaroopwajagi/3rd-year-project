...
@Component({
  selector: 'app-oom',
  templateUrl: './oom.component.html',
  styleUrls: ['./oom.component.css']
})
export class OomComponent implements OnInit {
 ...
 uploadata(books:any){
    ...
    console.warn(books);
    this.add.oom(books).subscribe((res)=>{
 ...
 ngOnInit(): void {
   this.display=this.add.getoom().subscribe(res=>{
     this.product = res;
