...
@Component({
  selector: 'app-sepm',
  templateUrl: './sepm.component.html',
  styleUrls: ['./sepm.component.css']
})
export class SepmComponent implements OnInit {
 ...
 uploadata(books:any){
    ...
    console.warn(books);
    this.add.sepm(books).subscribe((res)=>{
 ...
 ngOnInit(): void {
   this.display=this.add.getsepm().subscribe(res=>{
     this.product = res;
