...
@Component({
  selector: 'app-dspd',
  templateUrl: './dspd.component.html',
  styleUrls: ['./dspd.component.css']
})
export class DspdComponent implements OnInit {
 ...
 uploadata(books:any){
    ...
    console.warn(books);
    this.add.dspd(books).subscribe((res)=>{
 ...
 ngOnInit(): void {
   this.display=this.add.getdspd().subscribe(res=>{
     this.product = res;
